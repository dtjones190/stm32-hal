...
impl Qspi {
    pub fn new(regs: QUADSPI, cfg: QspiConfig, clocks: &Clocks) -> Result<Self> {
        assert!(
            cfg.dummy_cycles < 32,
    ...
    pub fn write_indirect(&mut self, addr: u32, data: &[u8]) -> Result<()> {
    ...
    pub fn read_indirect(&mut self, addr: u32, buf: &mut [u8]) -> Result<()> {
        bounded_loop!(self.is_busy(), Error::RegisterUnchanged);
        ...
        if (self.regs.sr().read().flevel().bits() as usize) < buf.len() {
            return Err(Error::QspiError(QspiError::Underflow));
        }
    ...
    pub fn read_1_mem_mapped(&mut self, offset: isize) -> Result<()> {
